import { ArrowRight, Terminal, Download, FileText, Code2, Cpu } from "lucide-react";
import { Button } from "@/components/ui/button";
import headshot from "@assets/headshot_Jaiden-Wilson_sq_1772998906594.jpg";

export function Hero() {
  return (
    <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-24 border-b border-border">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid lg:grid-cols-[1fr_350px] gap-12 lg:gap-16 items-start max-w-6xl mx-auto">
          ...
          <div className="space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 border border-border bg-card text-sm font-mono font-medium hard-shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 font-mono">
              ...
              {/* Optional: Add resume download link if a file is provided later */}
              {/* <Button size="lg" variant="outline" className="rounded-none h-12 px-8 border-2 border-foreground hover:bg-foreground hover:text-background transition-colors">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-8 border-t border-border mt-8">
               <div>
               ...
               <div className="col-span-2 sm:col-span-2 flex items-center gap-3 border-l border-border pl-4">
                  <div className="flex flex-col gap-2">
                     <span className="flex items-center gap-1 text-xs font-mono font-bold"><Code2 className="w-3 h-3"/> MERN Stack</span>
                     <span className="flex items-center gap-1 text-xs font-mono font-bold"><Cpu className="w-3 h-3"/> CompTIA A+</span>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="mx-auto lg:mx-0 w-64 h-64 sm:w-80 sm:h-80 lg:w-full lg:h-[400px] relative order-first lg:order-last group">
            {/* Cleaner, more professional image frame */}
            <div className="absolute inset-0 bg-primary translate-x-3 translate-y-3 border border-border transition-transform group-hover:translate-x-4 group-hover:translate-y-4"></div>
            <div className="absolute inset-0 border-2 border-foreground bg-background overflow-hidden p-2 z-10">
               <div className="w-full h-full relative overflow-hidden bg-muted">
                  <img 
                    src={headshot} 
                    alt="Jaiden Wilson - Full Stack Developer" 
